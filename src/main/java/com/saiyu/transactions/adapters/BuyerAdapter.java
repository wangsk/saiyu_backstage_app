...
public class BuyerAdapter extends RecyclerView.Adapter<BuyerAdapter.MyHolder>{
    List<BuyerRet.DatasBean> mItems;
    private OnItemClickListener mOnItemClickListener;

    public BuyerAdapter(List<BuyerRet.DatasBean> list){
        if(list == null){
            return;
        }
        this.mItems = list;
    ...
    @NonNull
    @Override
    public BuyerAdapter.MyHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_deal_layout, viewGroup, false);
        return new BuyerAdapter.MyHolder(view);
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull BuyerAdapter.MyHolder myHolder, final int i) {
        ...
        myHolder.tv_deal_3.setText(mItems.get(i).getBuyerAccount());
